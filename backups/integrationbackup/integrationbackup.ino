...
float theta_left = .3096;
float theta_right = .2817;
float beta_left = -27.78;
float beta_right = -30.78;
float v_star = 73.0;
...
float driveStraight_left(float v_star, float delta) {
  return (v_star+beta_left+delta*k_left)/theta_left;
}

float driveStraight_right(float v_star, float delta) {
  return (v_star+beta_right+delta*k_right)/theta_right;
}
...
float delta_reference(int k) {
  ...
  if (drive_mode == DRIVE_RIGHT) {
    return (CAR_WIDTH*v_star*k)/(5*TURN_RADIUS);
  }
